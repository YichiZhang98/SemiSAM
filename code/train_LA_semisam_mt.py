...
parser.add_argument('--exp', type=str,  default='SemiSAM-MT-2', help='model_name')
...
parser.add_argument('--label_num', type=str,  default=2, help='number of labeled cases')
### costs
...
if __name__ == "__main__":
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            ema_inputs = volume_batch + noise   # ema_inputs with noise

            outputs = model(volume_batch)
            outputs_soft = F.softmax(outputs, dim=1)


            samseg_mask = finetune_model_predict3D(
                volume_batch, label_batch.unsqueeze(1), sam_model_tune, device=device,
                click_method= get_next_click3D_torch_ritm, num_clicks=10, 
                prev_masks=None)    ##### Pseudo label generation with SAM    ###  outputs_soft[:,0:1,:,:,:]
            ...
            preds = torch.mean(preds, dim=0)  #(batch, 2, 128, 128, 128)
            ...
            consistency_weight_sam = get_current_consistency_weight((args.max_iterations - iter_num )//150)    ######  Decrease during training

            consistency_dist = consistency_criterion(outputs, ema_output) #(batch, 2, 128, 128, 128)
            ...
            sam_dice = losses.dice_loss(label_batch[:labeled_bs] == 1, samseg_soft[:labeled_bs] == 1) 

            sam_consistency = torch.mean(
                (outputs_soft[labeled_bs:] - samseg_soft[labeled_bs:])**2)
            sam_con_loss = 0.5 * consistency_weight_sam * sam_consistency

            consistency_loss = consistency_weight * consistency_dist
            loss = (loss_seg+loss_seg_dice) + consistency_loss + sam_con_loss
            ...
            logging.info('iteration %d : loss : %f cons_dist: %f, sam_dice_l: %f seg_dice_l: %f' %
                         (iter_num, loss.item(), consistency_dist.item(), 1-sam_dice, 1-loss_seg_dice))
            if iter_num % 50 == 0:
                ...
                image = torch.max(samseg_soft[-1, :, :, :, 20:61:10], 0)[1].permute(2, 0, 1).data.cpu().numpy()
                image = utils.decode_seg_map_sequence(image)
                grid_image = make_grid(image, 5, normalize=False)
                writer.add_image('unlabel/SAMseg_label', grid_image, iter_num)
