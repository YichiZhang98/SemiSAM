...
def compute_epistemic_uncertainty(all_preds):
    ...
    predictions = torch.stack(all_preds)  # (K, B, C, D, H, W)
    unc_map = torch.var(predictions, dim=0)  # (B, C, D, H, W)
    return unc_map
...
def finetune_model_predict3D_unc(img3D, gt3D, sam_model_tune, device='cuda', click_method='random', num_clicks=10, prev_masks=None):
    ...
    for b in range(batch_size):
        ...
        prev_seg_prob = torch.zeros_like(img3D_single).to(device)
        for num_click in range(num_clicks):
            with torch.no_grad():
                if num_click > 1:
                    click_method = get_next_click3D_torch_2
                batch_points, batch_labels = click_method(prev_seg_prob.to(device), gt3D_single.to(device))
...
def finetune_model_predict3D_mask(img3D, gt3D, sam_model_tune, device='cuda'):
    ...
    for b in range(batch_size):
        ...
        with torch.no_grad():
            ...
            medsam_seg_prob = torch.sigmoid(masks)
            medsam_seg = (medsam_seg_prob > 0.5).to(torch.uint8)
